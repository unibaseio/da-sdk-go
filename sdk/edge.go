...
func RegisterEdge(baseUrl string, auth types.Auth, em types.EdgeMeta) error {
	...
	_, err = doRequest(context.TODO(), baseUrl, "/v1/registerEdge", "", auth, strings.NewReader(form.Encode()))
	if err != nil {
		return err
	}
...
func GetEdge(baseUrl string, auth types.Auth, eaddr common.Address) (types.EdgeReceipt, error) {
	...
	resByte, err := doRequest(context.TODO(), baseUrl, "/v1/getEdge", "", auth, strings.NewReader(form.Encode()))
	if err != nil {
		return res, err
	}
...
func ListEdge(baseUrl string, auth types.Auth, filter string) (types.ListEdgeResult, error) {
	...
	resByte, err := doRequest(context.TODO(), baseUrl, "/v1/listEdge", "", auth, strings.NewReader(form.Encode()))
	if err != nil {
		return res, err
	}
